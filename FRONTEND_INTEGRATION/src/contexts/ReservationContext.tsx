// =====================================================
// src/contexts/ReservationContext.tsx  ← REMPLACER L'ANCIEN
  ...
  const getDashboardStats = () => {
    ...
    const topCars = Array.from(carMap.entries())
      ...
      .map(([brand, count]) => ({
        brand,
        count,
        model: brand.split(' ').slice(1).join(' '),
      }));
    ...
    const totalCompleted = allReservations.filter((r) => r.status === 'CONFIRMED').length;

    return {
      monthlyReservations,
      topCars,
      availability: {
        available: 0,  // à récupérer depuis /api/cars?available=true
        booked: active.length,
        total: active.length + getReservationHistory().length,
      },
      totalSpent,
      totalCompleted,
    };
